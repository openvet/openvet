...
class Consultation:
    ...
    def GetCritereGrade(self,idCritere,Valeur): 
        res=self.DBase.GetDbText("CALL GetCritereGrade(%i,%f)"%(idCritere,Valeur))
        if len(res)>0:
            Grade=res[0]+QtCore.QString('/')+res[1]
        return Grade
    
    def SaveCritere(self,index,ids,valeur,grade):
        ...
        data=[]
        for i in ids:
            data.append(i.toInt()[0])
        data.extend([self.ConsultationPathologies[index].idPathologieRef,None,None,grade])
        tmp=CritereConsultation(self.ConsultationPathologies[index])
        tmp.Set(data,valeur)
        tmp.Save()
        del tmp
    ...
    def Save(self):
        ...
        if self.Examen.size()>65536:
            ...
            values.append('\"%s\"'%self.Examen.toUtf8().data())
        if self.Traitement.size()>65536:
            erreurs.append('Traitement')
        else:
            values.append('\"%s\"'%self.Traitement.toUtf8().data())
        values.append('TRUE')
        if self.Commentaires is None:
            ...
            values.append('\"%s\"'%self.Commentaires.toUtf8().data())
        if ToDelete:
