...
#TODO paramétrer nom base
class Table:
    
    def __init__(self,name):
        #self.parent
        try:
            self.con=mdb.connect('localhost','root','horizons','OpenVet12',use_unicode=True, charset="utf8")
        except mdb.Error, e:
            print "Error %d: %s" % (e.args[0], e.args[1])
            sys.exit(1)
        #self.con.set_character_set('utf8')
        self.champs=[]
        self.table='kiwi_compta.'+name
        self.getfields()
    ...
    def check_doublon(self,data):
        ...
        if self.table=='kiwi_compta.Banque':
        	req=[(data[1],1),(data[4],4)]
        	res=self.select(req)
        	if len(res)>0:
        		error='Doublon Banque:%i'%res[0][0]
        if self.table=='Kiwi_compta.depenses':
    ...
    def validcol(self,data,index):  #TODO Rewrite clean
        error=None
        if data==None:
            return error
        ...
        if isinstance(self.champs[index][1],int):
            ...
            if str(self.champs[index][1])=="<type 'datetime.date'>":
                ...
                if not isinstance(data,self.champs[index][1]):
                    if not (str(self.champs[index][1])=="<type 'float'>" and isinstance(data,Decimal)):
...
def execute(query):
    try:
        con=mdb.connect('localhost','root','horizons','OpenVet12')
    except mdb.Error, e:
        print "Error %d: %s" % (e.args[0], e.args[1])
        sys.exit (1)
...
def savedata(query):
    try:
        con=mdb.connect('localhost','root','horizons','OpenVet12',use_unicode=True, charset="utf8")
    except mdb.Error, e:
        print "Error %d: %s" % (e.args[0], e.args[1])
        sys.exit (1)       
    error=None
