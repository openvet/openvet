...
class TableVille(TableSelectAll):
    ...
    QModel=''
    ListeProxyModel=[] 
    def __init__(self, initialiseListe=False, idPays=33) :
        nomTable='Commune'
...
#        sql="""SELECT   CONCAT ( Commune, ' (',CIP , ')' )
#        FROM Commune
#        WHERE Actif  AND Pays_idPays = """+str(idPays)
        
#        sql="SELECT *  FROM Commune" #debug
        
        TableSelectAll. __init__(self, nomTable, self.__class__.ListeElement, self.__class__.ListeId, self.__class__.QModel,self.__class__.ListeProxyModel,  initialiseListe, sql)

def NouvelleTableVille(initialiseListe=False, idPays=33):
    return TableVille(initialiseListe, idPays)
